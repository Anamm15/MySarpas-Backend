...
const getPeminjamanByUserId = async (req, res) => {
  ...
  try {
    const peminjaman = await prisma.peminjaman.findMany({
        where: { User_idUser: parseInt(idUser) },
        skip,
        take: limit,
        orderBy: {
          tanggal: 'desc',
    ...
    res.json({
      data: peminjaman,
      current_page: page,
      total_pages: Math.ceil(peminjaman.length / limit),
      total_data: peminjaman.length,
      per_page: limit
    });
  } catch (error) {
    console.error('Get peminjaman error:', error);
...
const getPeminjamanByRuanganId = async (req, res) => {
    ...
    if (!tanggal) {
      return res.status(400).json({ error: 'Tanggal harus disertakan dalam query parameter' });
    }
...
const createPeminjaman = async (req, res) => {
    ...
    if (!ruanganId || !tanggal || !jamAwal || !jamAkhir || !jenisKegiatan) {
    ...
    if (tanggalDate < minBookingDate) {
        return res.status(400).json({
            error: 'Peminjaman harus dilakukan minimal 3 hari sebelum tanggal pelaksanaan',
        });
    }
